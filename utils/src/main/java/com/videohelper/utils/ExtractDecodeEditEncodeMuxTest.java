...
public class ExtractDecodeEditEncodeMuxTest{
    ...
    private static  int OUTPUT_AUDIO_SAMPLE_RATE_HZ = 48000; // Must match the input stream.
    ...
    public ExtractDecodeEditEncodeMuxTest(Activity activity){
        mActivity = activity;
    }

    public void ExtractDecodeEditEncodeMuxAudioVideo(String sourcePath, int outputWidth,int outputHeight,byte[] overlayImages, int numsOfImages)throws Throwable {
        mPath = sourcePath;
        setSize(outputWidth,outputHeight);
        setCopyAudio();
        setCopyVideo();
        extractDecodeEditEncodeMux(false);
    }
     ...
     */
    private void extractDecodeEditEncodeMux(boolean isResSource) throws Exception {
        ...
        try {
            ...
            if (mCopyVideo) {
                mVideoExtractor = createExtractor(isResSource);
                ...
                String format = inputFormat.getString(MediaFormat.KEY_COLOR_FORMAT);
            ...
            if (mCopyAudio) {
                mAudioExtractor = createExtractor(isResSource);
                ...
                try {
                    ...
                    throw new Exception("Can't get channel count and sample rate from input");
                }
     ...
     */
    private MediaExtractor createExtractor( boolean isFromRes) throws IOException {
        MediaExtractor extractor;
        if(isFromRes){
            AssetFileDescriptor srcFd = mActivity.getResources().openRawResourceFd(mSourceResId);
        ...
        MediaCodec.Callback callback = new MediaCodec.Callback() {
            ...
            public void onOutputBufferAvailable(MediaCodec codec, int index, MediaCodec.BufferInfo info) {
                ...
                if (render) {
                    ...
                    mOutputSurface.drawImage(frameCounter++);
